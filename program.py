        ...
        if self.force_annotation or self.windowName is not None:
            ...
            if self.windowName:
                if os.name == 'nt':
                    cv2.waitKey(1)
                diff = 100
                start = (annotated_im.shape[1]//2-diff,annotated_im.shape[0]//2-diff)
                end = (annotated_im.shape[1]//2+diff,annotated_im.shape[0]//2+diff)
                cv2.rectangle(annotated_im, start,end, (0,255,255),10)
